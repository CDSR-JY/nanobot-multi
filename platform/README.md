...
| `PLATFORM_VLLM_API_BASE` | _(空)_ | vLLM 服务地址，如 `http://vllm-host:8000/v1` |
| `PLATFORM_VLLM_API_KEY` | _(空)_ | vLLM 可选 API Key，不需要可留空 |
| `PLATFORM_DEFAULT_MODEL` | `claude-sonnet-4-5` | 新用户默认模型（用 vLLM 时设为 `vllm/模型名`） |
