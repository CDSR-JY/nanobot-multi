...
def _resolve_provider(model: str) -> tuple[str, str, str | None]:
    ...
    # vLLM / local OpenAI-compatible server (model must be "vllm/..." when PLATFORM_VLLM_API_BASE is set)
    if model_lower.startswith("vllm/") and settings.vllm_api_base:
        local_model = model.split("/", 1)[1]  # e.g. Qwen2.5-7B-Instruct
        # LiteLLM with api_base uses openai/ model name
        return f"openai/{local_model}", (settings.vllm_api_key or "dummy"), settings.vllm_api_base
    ...
    # Fallback: vLLM — when API base is set, any unknown model is sent to vLLM (e.g. minimax-m2.1)
    if settings.vllm_api_base:
        return f"openai/{model}", (settings.vllm_api_key or "dummy"), settings.vllm_api_base
