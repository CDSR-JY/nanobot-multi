...
def _extract_internal_ip(docker_container: docker.models.containers.Container) -> str:
    """Get container IP on configured internal network (fallback to any non-empty IP)."""
    docker_container.reload()
    networks = docker_container.attrs.get("NetworkSettings", {}).get("Networks", {})
    internal_ip = networks.get(settings.container_network, {}).get("IPAddress", "")
    if internal_ip:
        return internal_ip
...
async def _sync_container_endpoint(
    db: AsyncSession,
    record: Container,
    docker_container: docker.models.containers.Container,
) -> None:
    """Sync DB endpoint to the container's current internal IP/port."""
    current_ip = _extract_internal_ip(docker_container)
    updates: dict[str, object] = {}

    if current_ip and current_ip != record.internal_host:
        updates["internal_host"] = current_ip
        record.internal_host = current_ip

    if record.internal_port != 18080:
        updates["internal_port"] = 18080
...
async def ensure_running(db: AsyncSession, user_id: str) -> Container:
    ...
    docker_container: docker.models.containers.Container | None = None

    if record.status == "paused":
        try:
            docker_container = client.containers.get(record.docker_id)
    ...
    if docker_container is not None:
        await _sync_container_endpoint(db, record, docker_container)
